...
int find_word(const char* string, int* index) {
	while (string[(*index)] != '\0') {
		if ((*index) == 0) {
			if (is_letter(string[(*index)]) != 0) {
				return (*index);
			}
		}
		else if ((*index) > 0) {
			if (string[(*index) - 1] != '-' && is_letter(string[(*index) - 1]) == 0 && is_letter(string[(*index)]) != 0) {
				return (*index);
			}
		}
		(*index)++;
...
char* word_from_dictionary(words** array, const char* word, const int* size) {
	char* new_word = NULL;
	for (int i = 0; i < (*size); i++) {
		if (!strcmp(word, (*array)[i].word1)) {
			new_word = (*array)[i].word2;
			break;
		}
		if (!strcmp(word, (*array)[i].word2)) {
			new_word = (*array)[i].word1;
			break;
		}
	}
	return new_word;
...
void input_dictionary(words** array, const char* str, int* size) {
	words word;
	...
	word.word1 = word1;
	word.word2 = word2;
	input_words_in_array(word, array, size);
}

void input_word_from_dictionary(char** str, const char* word, const char* new_word, int* index) {
	if (new_word == NULL) {
		return;
	}
...
void input_file(const char* str) {
	FILE* file = NULL;
	errno_t err;

	err = fopen_s(&file, "C:\\programming\\C\\zipper\\razarhfile.txt", "a");
	error_check(err);
...
void unzipper(words** array, int* size) {
	...
	err = fopen_s(&file, "C:\\programming\\C\\zipper\\arhfile.txt", "r");
	error_check(err);

	fgets(buf, 4096, file);
	while (buf[0] != '$') {
		input_dictionary(array, buf, size);
		fgets(buf, 4096, file);
	}
	fgets(buf, 4096, file);
	while (!feof(file)) {
